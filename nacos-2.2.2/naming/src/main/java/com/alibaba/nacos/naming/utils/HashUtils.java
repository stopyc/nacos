...
public class HashUtils {
    ...
    public SortedMap<Object, Object> getHashRing(List<? extends Instance> realNodes, int virtualNodesCount) {
        ...
        for (Instance realNode : realNodes) {
            ...
            SortedMap<Integer, String> sortedMap = getOneHashRing(realNode.getIp(), realNode.getPort(), 5);
     ...
     * @return :treeMap
     */
    private SortedMap<Integer, String> getOneHashRing(String ip, int port, int virtualNodesCount) {
        ...
        System.out.println("The hash of the real node of the ws service with IP port " + ip + ": " + port + " is: " + realNodeHash);
    ...
    public void updateHashRingIsRegister(String namespaceId, String serviceName, String ip, int port, boolean healthy, boolean enabled)  {
        ...
        System.out.println(serviceName + "请求服务注册, ip端口为: " + ip + ":" + port + "是否健康: " + (healthy ? "是" : "否") + "是否可用: " + (enabled ? "是" : "否"));

        if (serviceName.contains(getWsServiceName())) {

            SortedMap<Integer, String> sortedMap = getOneHashRing(ip, port, 5);

            SortedMap<Object, Object> map = int2ObjMap(sortedMap);

            System.out.println("开始添加" + serviceName + "ip端口为:" + ip + ":" + port + "的哈希环");
            redisTemplate.opsForHash().putAll(getHashRingKey(), map);

            System.out.println("通知gateway有ws服务上线");
